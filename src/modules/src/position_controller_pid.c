...
// for logging
static float xVelRef_PID = 0.0;
static float yVelRef_PID = 0.0;
static float xVel_PID = 0.0;
static float yVel_PID = 0.0;
...
static float rpLimit = 20; // keep the same as in mpc!
static float rpLimitOverhead = 1.10f;
// Velocity maximums
static float xyVelMax = 2.0f; // CHANGED from 1.0 to 2.0 -> to check for stability
...
#ifndef UNIT_TEST
static struct this_s this = { .pidVX = { .init = { .kp = 25.0f, .ki = 1.0f,
		.kd = 0.0f, }, .pid.dt = DT , },
...
.pidX = { .init = { .kp = 2.0f, .ki = 0, .kd = 0, }, .pid.dt = DT , },

.pidY = { .init = { .kp = 2.0f, .ki = 0, .kd = 0, }, .pid.dt = DT , },
...
void velocityController(float* thrust, attitude_t *attitude,
	...
	// logging
	xVelRef_PID = setpoint->velocity.x;
	yVelRef_PID = setpoint->velocity.y;
	xVel_PID = state->velocity.x;
	yVel_PID = state->velocity.y;

	// Roll and Pitch
	float rollRaw = runPid(state->velocity.x, &this.pidVX, setpoint->velocity.x,
			DT);
...
LOG_GROUP_START(PID_LOGGING)
LOG_ADD((LOG_FLOAT), x_vel ,&xVel_PID)
LOG_ADD((LOG_FLOAT), y_vel ,&yVel_PID)
LOG_ADD((LOG_FLOAT), x_vel_ref ,&xVelRef_PID)
LOG_ADD((LOG_FLOAT), y_vel_ref ,&yVelRef_PID)
